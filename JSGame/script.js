...
window.GAME = (() => {
  ...
  const GameConst = {
    Fireball: {
      size: 30,
      getSpeed: (isLeftDirection) => isLeftDirection ? Math.random() * (3 - 1.2) + 1.2 : Math.random() * (2 - 1) + 1,
    },
    Player: {
      speed: 2,
      width: 150,
  ...
  const GAME_MESSAGES = {
    [GameStatus.WIN]: 'You have won!\nYeah!',
    [GameStatus.FAIL]: 'You have lost!',
  ...
  class PlayerObject extends FireballsThrowerObject {
    ...
    behave(gameState, timeframe) {
      ...
      const hit_fireball = fireballs.filter(fireball => ((this.x + 5 < fireball.x && fireball.x < this.x + this.width - 50) && (this.y < fireball.y && fireball.y < this.y + this.height)))[0];

      if (hit_fireball) {
        hit_fireball.state = ObjectState.DISPOSED;
  ...
  class FireballObject extends AbstractGameObject {
    ...
    gravity(horizontal_speed) {
    	this.y += 0.8 + this.speed/horizontal_speed;
    }
  }
  ...
  const GAME_RULES = [
     ...
     */
    (gameState) => {
      const player = gameState.objectsToDispose.filter((object) => object.type === GameObjectType.PLAYER)[0];
    ...
    /**
     * Demo condition: game round considering as a successful one if there're at least one disposed enemy object
     * @param {Object} gameState
     * @return {GameStatus}
     */
     
    (gameState) => {
      const deletedEnemies = gameState.objectsToDispose.filter(object => object.type === GameObjectType.ENEMY);
      return deletedEnemies.length ? GameStatus.WIN : GameStatus.CONTINUE;
    },
  ];
